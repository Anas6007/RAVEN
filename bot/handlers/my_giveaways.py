    ...
    for giveaway in items[start:end]:
        ...
        rows.append([
            InlineKeyboardButton(
                ...
                callback_data=f"giveaway_manage:{giveaway.id}",
            )
        ])

    nav = []
    if page > 1:
        nav.append(InlineKeyboardButton("⬅️", callback_data=f"my_giveaways_page:{page-1}"))
...
async def my_giveaways(client, callback):
    await callback.answer()

    async with SessionLocal() as db:
        giveaways = GiveawayRepository(db)
        items = await giveaways.get_by_owner(callback.from_user.id, flow_kind="giveaway")

        if not items:
            await callback.message.edit_text(
                f"""
{flow_main_list_title(type('obj', (), {'flow_kind': 'giveaway'})())}

━━━━━━━━━━━━━━

{flow_empty_list_text(type('obj', (), {'flow_kind': 'giveaway'})())}
""",
                reply_markup=_back_keyboard(),
            )
            return

        text = f"{flow_main_list_title(type('obj', (), {'flow_kind': 'giveaway'})())}\\n\\n━━━━━━━━━━━━━━\\n\\n"
        text += f"إجمالي السحوبات: **{len(items)}**\\n\\n"
...
async def my_giveaways_page(client, callback):
    ...
    if not items:
        await callback.answer("لا توجد سحوبات.", show_alert=True)
        return
...
async def giveaway_manage(client, callback):
    ...
    async with SessionLocal() as db:
        ...
        noun = "المسابقة" if getattr(giveaway, "flow_kind", "giveaway") == "competition" else "السحب"

        text = f"""
📋 **تفاصيل {noun} #{giveaway.id}**
...
"""
        ...
        await callback.message.edit_text(
            ...
            reply_markup=manage_giveaway_keyboard(
                giveaway_id,
                giveaway.is_active,
                getattr(giveaway, "flow_kind", "giveaway"),
            ),
        )


def register(app):
    app.add_handler(CallbackQueryHandler(my_giveaways, filters.regex("^my_giveaways$")))
    app.add_handler(CallbackQueryHandler(my_giveaways_page, filters.regex(r"^my_giveaways_page:(\\d+)$")))
    app.add_handler(CallbackQueryHandler(giveaway_manage, filters.regex(r"^giveaway_manage:(\\d+)$")))
